...
class MeasurementFromSampleForm(MeasurementFormMixin, forms.ModelForm):
    ...
    measure_date = forms.DateTimeField(
        widget=DatePickerInput(options={'format'         : '%Y-%m-%d %H:%M',
                                        "showTodayButton": True, }), required=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user.first_name and user.last_name:
            self.exp_title = _('New measurement of a sample by %(first)s %(last)s') % {'first': user.first_name,
                                                                                       'last' : user.last_name}
